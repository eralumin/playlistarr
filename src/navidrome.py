...
class NavidromeService:
    ...
    def create_playlist(self, playlist_name) -> NavidromePlaylist | None:
        ...
        if response.status_code == 200:
            ...
            logging.error(f'Failed to create playlist {playlist_name}: {response.content}')
            return None

    def get_or_create_playlist(self, playlist_name) -> str | None:
        playlist = self.get_playlist_or_none(playlist_name)
        if playlist:
            logging.info(f'Playlist "{playlist_name}" already exists with ID {playlist._id}.')
            return playlist
        else:
    ...
    def clear_playlist(self, playlist: NavidromePlaylist):
        ...
        response = requests.get(url, params=params)
        if response.status_code == 200:
            logging.info(f'Successfully cleared all tracks from playlist {playlist.name}.')
        else:
            logging.error(f'Failed to clear playlist {playlist.name}: {response.content}')
    ...
    def add_tracks_to_playlist(self, playlist: NavidromePlaylist):
        ...
        response = requests.get(url, params=params)
        if response.status_code == 200:
            logging.info(f'Successfully added {len(track_ids)} tracks to playlist "{playlist.name}".')
        else:
            logging.error(f'Failed to add tracks to playlist "{playlist.name}": {response.content}')
